...
const fetchTopItuneAlbums = async () => {
  try {
    const response = await fetch(`${ITUNES_BASE_URL}us/rss/topalbums/limit=100/json`);
    if (!response.ok) throw new Error("Error fetching iTunes data");
    const results = await response.json();
    return results;
  } catch (err) {
    console.error(err);
...
const fetchSongs = async (albumId: string) => {
  try {
    const response = await fetch(`${ITUNES_BASE_URL}lookup?id=${albumId}&entity=song`);
    if (!response.ok) throw new Error("Error fetching song preview");
    const data = await response.json();
    return data;
  } catch (error) {
